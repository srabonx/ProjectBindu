...
namespace HM {
	...
	inline std::string RelativeResourcePath(const std::string& relativeFilePath)
	{
		std::filesystem::path currPath = std::filesystem::current_path();

		int trimCount{ 0 };

		for (char i : relativeFilePath)
		{
		...
		auto trimmedPath = relativeFilePath.substr(trimCount);

		std::filesystem::path relativePath(trimmedPath);
